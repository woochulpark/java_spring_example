...
function BonbuIn_Add()
{
	var form = document.BonbuInForm;
	form.action = "BonbuIn_Add.do";
	form.submit();
}
function BonbuIn_DETAIL(DOCNO)
{
...
<body class="home image-sphere-style responsive">
	<form:form name="BonbuInForm" method="post" action="BonbuInList_search.do">
		...
		<div class="wrapper group">
			...
			<div class="subtitle" style="text-align: right;">입출고 관리 > 본부 입고</div>
			<h2>본부 입고</h2>
			...
			<div style="text-align:right;">
				<a href="javascript:BonbuIn_Add()" class="search grey button">본부입고</a>
			</div>
			</c:if>
			<!-- START TABLE -->
				...
				<table id='nTbl'>
					<thead>
						<tr>
							...
							<th style="width: 14%;">대분류</th>
							<th style="width: 15%">중분류</th>
							<th style="width: 15%">소분류</th>
							<th style="width: 12%">제조일자</th>
							<th style="width: 12%">물류입고일자</th>
							<th style="width: 12%">물류출고일자</th>
							<th style="width: 10%">출고 예상량</th>
						</tr>
					</thead>
					<tbody>
					<c:if test="${BonbuInListSize > 0 }">
					<c:forEach var="BonbuInList" items="${BonbuInList}" varStatus="st">
						<tr style="height: 40px;" onclick="javascript:BonbuIn_DETAIL('${BonbuInList.DIL_DOCNO}')">
							<td>${BonbuInList.DIL_MODEL}</td>
							<td>${BonbuInList.MMC_LGROUPNAME}</td>
							<td>${BonbuInList.MMC_MGROUP}</td> 	
							<td>${BonbuInList.MMC_SGROUP}</td>
							<td>${BonbuInList.DIL_JEJO}</td> 	 
							<td>${BonbuInList.DIL_INDate}</td>
