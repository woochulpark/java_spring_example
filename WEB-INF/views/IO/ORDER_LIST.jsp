...
{
	var form = document.orderForm;
	form.action = "Order_Add.do";
	form.submit();
}
function Order_Detail(DOCNO)
{
...
<body class="home image-sphere-style responsive">
	<form:form name="orderForm" method="post" action="OrderList_search.do">
		...
		<div class="wrapper group">
			...
			<div class="subtitle" style="text-align: right;">입출고 관리 > 본부발주</div>
			<h2>본부발주</h2>
			...
			<div id="searchfield">
				<table style="width: 700px;">
					...
					<tr>
						<td style="text-align: center;">발주일자</td>
						<td>
			...
			<div style="text-align:right;">
				<a href="javascript:Order_Add()" class="search grey button">신 규</a>
			</div>
			</c:if>
			<!-- START TABLE -->
				...
				<table id='nTbl'>
					<thead>
						<tr>
							...
							<th style="width: 10%">발주자</th>
							<th style="width: 6%">발주수량</th>
							<th style="width: 6%">출고수량</th>
							<th style="width: 6%">본부 입고 확인</th>
						</tr>
					</thead>
					<tbody>
					<c:if test="${OrderListSize > 0 }">
					<c:forEach var="OrderList" items="${OrderList}" varStatus="st">
						<tr style="height: 40px;" onclick="javascript:Order_Detail('${OrderList.DBO_DOCNO}')">
							<td>${OrderList.MCI_BonbuNAME}</td>
							<td>${OrderList.MCI_CenterName}</td>
							<td>${OrderList.DBO_DOCNO}</td> 	
							<td>${OrderList.DBO_MODEL}</td>
							<td>${OrderList.MMC_LGROUPNAME}</td> 	 
							<td>${OrderList.MMC_MGROUP}</td> 	 	 
							<td>${OrderList.MMC_SGROUP}</td> 	
							<td>${OrderList.DBO_DATE}</td> 	
							<td>${OrderList.DBO_USERNAME}</td> 	
							<td>${OrderList.DBO_QTY}</td> 	
							<td>
								<c:if test="${OrderList.DBM_Sts == 'Y'}">
									${OrderList.DBM_QTY}
								</c:if>
								<c:if test="${OrderList.DBM_Sts == 'N'}">
									미출고
								</c:if>
							</td> 	
							<td>
								<c:if test="${OrderList.DBO_BisSts == 'Y'}">
									${OrderList.DBO_CFMDATE}
								</c:if>
								<c:if test="${OrderList.DBO_BisSts == 'N'}">
									미입고
								</c:if>
							</td> 	  	 		 	
						</tr>
