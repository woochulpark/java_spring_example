...
$(document).ready(function(){
	...
	$("#OutStartDay").datepicker();
	$("#OutEndDay").datepicker();
	$("#InStartDay").datepicker();
	$("#InEndDay").datepicker();
});
function search()
{
	var form = document.inoutForm;
	form.submit();
}
...
function BonbuMove_MODIFY(DOCNO)
{
	document.getElementById("DOCNO").value = DOCNO;
	var form = document.inoutForm;
	form.action = "BonbuMove_MODIFY.do";
	form.submit();
}
...
<body class="home image-sphere-style responsive">
	<form:form name="inoutForm" method="post" action="BonbuMove_List_Search.do">
		...
		<div class="wrapper group">
			...
			<div class="subtitle" style="text-align: right;">입출고 관리 > 본부별 이동</div>
			<h2>본부별 이동</h2>
				...
				<table style="width: 80%;">
					<tr>
						...
						<td colspan="3" style="width: 75%;">
							<input type="text" name="OutStartDay" id="OutStartDay" value="${OutStartDay}"  style="width:40%;" readonly="readonly">
							&nbsp;~&nbsp;
							<input type="text" name="OutEndDay" id="OutEndDay" value="${OutEndDay}"  style="width:40%;" readonly="readonly">
						</td>
					</tr>
					<tr>
						<td style="text-align: center;width: 25%;">입고일자</td>
						<td colspan="3" style="width: 75%;">
							<input type="text" name="InStartDay" id="InStartDay" value="${InStartDay}"  style="width:40%;" readonly="readonly">
							&nbsp;~&nbsp;
							<input type="text" name="InEndDay" id="InEndDay" value="${InEndDay}"  style="width:40%;" readonly="readonly">
						</td>
					</tr>
					<c:choose>
					...
					<tr>
						...
						<td colspan="3">
							<input type="radio" name = "CTYPE" value = "" <c:if test="${empty CTYPE}">checked="checked"</c:if>> 전체
							<input type="radio" name = "CTYPE" value = "Y" <c:if test="${CTYPE == 'Y'}">checked="checked"</c:if>> 입고
							<input type="radio" name = "CTYPE" value = "N" <c:if test="${CTYPE == 'N'}">checked="checked"</c:if>> 출고
						</td>
					</tr>
					<tr>
				...
				<table id='nTbl'>
					<thead>
						<tr>
							...
							<th style="width: 8%;">출고본부</th>
							<th style="width: 7%">센 터</th>
							<th style="width: 8%;">입고본부</th>
							<th style="width: 7%;">센 터</th>
							<th style="width: 7%">발주일</th>
							<th style="width: 8%">출고일</th>
							<th style="width: 8%">입고일</th>
							<th style="width: 7%">입고상태</th>
						</tr>
					</thead>
					<tbody>
					<c:if test="${MoveListSize > 0 }">
					<c:forEach var="MoveList" items="${MoveList}" varStatus="st">
						<tr style="height: 40px;" onclick="javascript:BonbuMove_MODIFY('${MoveList.DBO_DOCNO}')">
							<td style="padding: 2px;">${MoveList.DBO_DOCNO}</td>
							<td style="padding: 2px;">${MoveList.DBO_MODEL}</td>
							<td style="padding: 2px;">${MoveList.MMC_LGROUPNAME}</td>
							<td style="padding: 2px;">${MoveList.MMC_MGROUP}</td> 	
							<td style="padding: 2px;">${MoveList.MMC_SGROUP}</td>
							<td style="padding: 2px;">${MoveList.OUTBONBUNAME}</td> 	 
							<td style="padding: 2px;">${MoveList.OUTCENTERNAME}</td> 	 	
							<td style="padding: 2px;">${MoveList.INBONBUNAME}</td> 	 
							<td style="padding: 2px;">${MoveList.INCENTERNAME}</td> 	 	 
							<td style="padding: 2px;">${MoveList.DBO_DATE}</td> 	
							<td style="padding: 2px;">${MoveList.DBM_DATE}</td> 	
							<td style="padding: 2px;">${MoveList.DBO_CFMDATE}</td> 	
							<td style="padding: 2px;">
								<c:if test="${MoveList.DBM_Sts == 'N'}">
									미출고
								</c:if>
								<c:if test="${MoveList.DBM_Sts == 'Y' and MoveList.DBO_BisSts == 'N'}">
									입고중
								</c:if>
								<c:if test="${MoveList.DBO_BisSts == 'Y'}">
									입고완료
								</c:if>
							</td> 	  	 		 	
						</tr>
