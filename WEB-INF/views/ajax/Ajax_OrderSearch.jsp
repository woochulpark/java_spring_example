...
<div class="scroll_table">
	<table id='nTbl'>
		...
		<tbody>
			...
			<tr style="height: 40px;" onclick="javascript:AddModel('${ModelList.MMC_CODE}', '${ModelList.MMC_LGROUPNAME}', '${ModelList.MMC_MGROUP}', '${ModelList.MMC_SGROUP}')">
...
<a href="#" onClick="Ajax_Order_search(1)">[처음]</a> 
</c:if>
<c:if test="${pagination.curPage ne 1}">
<a href="#" onClick="Ajax_Order_search('${pagination.prevPage }')">[이전]</a> 
</c:if>
<c:forEach var="pageNum" begin="${pagination.startPage }" end="${pagination.endPage }">
<c:choose>
<c:when test="${pageNum eq  pagination.curPage}">
<span style="font-weight: bold;"><a href="#" onClick="Ajax_Order_search('${pageNum }')">${pageNum }</a></span> 
</c:when>
<c:otherwise>
<a href="#" onClick="Ajax_Order_search('${pageNum }')">${pageNum }</a> 
</c:otherwise>
</c:choose>
</c:forEach>
<c:if test="${pagination.curPage ne pagination.pageCnt && pagination.pageCnt > 0}">
<a href="#" onClick="Ajax_Order_search('${pagination.nextPage }')">[다음]</a> 
</c:if>
<c:if test="${pagination.curRange ne pagination.rangeCnt && pagination.rangeCnt > 0}">
<a href="#" onClick="Ajax_Order_search('${pagination.pageCnt }')">[끝]</a> 
</c:if>
</div>