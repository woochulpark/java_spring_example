...
<body class="home image-sphere-style responsive">
	<form:form name="statusForm" method="post" action="ModelBepum_Search.do">
		...
		<div class="wrapper group">
			...
			<div class="subtitle" style="text-align: right;">관리 > 비품 현황</div>
			<h2>비품 현황</h2>
				...
				<table style="width: 700px;">
					...
					<tr>
						...
						<td>
							...
							<input type="radio" name="BISSTS" value="1" <c:if test="${BISSTS == '1'}">checked="checked"</c:if>> 보관
							<input type="radio" name="BISSTS" value="2" <c:if test="${BISSTS == '2'}">checked="checked"</c:if>> 분실
							<input type="radio" name="BISSTS" value="3" <c:if test="${BISSTS == '3'}">checked="checked"</c:if>> 망실
						</td>
					</tr>
					<tr>
				...
				<table id='nTbl'>
					<thead>
						<tr>
							...
							<th style="width: 10%">재 고</th>
							<th style="width: 10%">상 태</th>
							<th style="width: 10%">비고</th>
						</tr>
					</thead>
					<tbody>
					<c:if test="${BepumListSize > 0 }">
					<c:forEach var="BepumList" items="${BepumList}" varStatus="st">
						<tr style="height: 40px;">
							<td>${BepumList.MCI_BonbuNAME}</td>
							<td>${BepumList.MCI_CenterName}</td>
							<td>${BepumList.MMC_CODE}</td>	 		 	
							<td>${BepumList.MMC_LGROUPNAME}</td>	 
							<td>${BepumList.MMC_MGROUP}</td>	 
							<td>${BepumList.MMC_SGROUP}</td>	 
							<td>${BepumList.DMJ_JEGO}</td>	 
							<td>${BepumList.MODEL_STS}</td>	 
							<td>${BepumList.CMNT}</td>
						</tr>
					</c:forEach>
					</c:if>
					</tbody>
